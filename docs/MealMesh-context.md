...
| Meal engine | **Local deterministic engine (no LLM at runtime)** | Plans are generated by MealMesh's OWN engine in `src/lib` (`generatePlan.ts` → `planEngine.ts`: a procedural base from `mockPlan.ts` overlaid with the tagged recipe corpus), gated by the same deterministic `validatePlan` safety pass (§4). No external model is called at runtime — generation is instant, free, offline-capable, and immune to LLM quota/latency/truncation failures. The recipe corpus lives in Supabase (`recipe_corpus`, §3) with an in-memory seed (`src/lib/corpus.ts`) as the offline fallback; corpus growth (seed + offline ingest of licensed datasets) is what scales dish variety. The old Gemini Edge Function (`supabase/functions/generate-plan`) is **deprecated** and no longer on the generation path — retained only as reference for the LLM-proxy pattern. |
